...
extends Resource
## Class with the different growth rates a pokemon can have.

@export var id: Constants.GROWTH_RATE: set = _set_growth_rate ## The id of the growth rate.
var _rate: GrowthRate ## The growth rate of the pokemon.

## Sets the right inner class to _rate, to apply the right formula.
func _set_growth_rate(p_id: Constants.GROWTH_RATE) -> void:
	id = p_id
	match id:
		Constants.GROWTH_RATE.FAST:
			_rate = Fast.new()
		Constants.GROWTH_RATE.MEDIUM_FAST:
			_rate = MediumFast.new()
		Constants.GROWTH_RATE.SLOW:
			_rate = Slow.new()
		Constants.GROWTH_RATE.MEDIUM_SLOW:
			_rate = MediumSlow.new()
		Constants.GROWTH_RATE.ERRATIC:
			_rate = Erratic.new()
		Constants.GROWTH_RATE.FLUCTUATING:
			_rate = Fluctuating.new()

## Gets the lookup table of experience for each table. To be overriden.
func _get_lookup_table() -> Array[int]:
	return []


## Applies the growth rate formula for the given level.
func apply_formula(level: int) -> int:
	if _rate == null:
		return 0
	return _rate.apply_formula(level)


## Gets the minimum experience required for the given level.
func get_level_minimum_exp(level: int) -> int:
	if _rate == null:
		return 0
	if level <= 0:
		print("Invalid level given.")
		return 0
	return _rate.apply_formula(level)


## Gets the experience for the max level.
func get_maximum_experience() -> int:
	return get_level_minimum_exp(100) # TODO: Move to a setting


## Gets the level that corresponds to the given experience.
func get_level_from_experience(experience: int) -> int:
	var max_exp: int = get_maximum_experience()
	if experience >= max_exp:
		return 100 # TODO: Move to a setting
	var lookup_table: Array[int] = _rate._get_lookup_table()
	for i in lookup_table.size():
		if experience <= lookup_table[i]:
			return maxi(i, 1)
	return 100 # TODO: Move to a setting


class Fast extends GrowthRate:
	func _get_lookup_table() -> Array[int]:
		return [
			0, 6, 21, 51, 100, 172, 274, 409, 583, 800,
			1_064, 1_382, 1_757, 2_195, 2_700, 3_276, 3_930, 4_665, 5_487, 6_400,
			7_408, 8_518, 9_733, 11_059, 12_500, 14_060, 15_746, 17_561, 19_511, 21_600,
	...
	func apply_formula(level: int) -> int:
		return (pow(level, 3) * 4) / 5


class MediumFast extends GrowthRate:
	func _get_lookup_table() -> Array[int]:
		return [
			0, 8, 27, 64, 125, 216, 343, 512, 729, 1_000,
			1_331, 1_728, 2_197, 2_744, 3_375, 4_096, 4_913, 5_832, 6_859, 8_000,
			9_261, 10_648, 12_167, 13_824, 15_625, 17_576, 19_683, 21_952, 24_389, 27_000,
...
class MediumSlow extends GrowthRate:
	func _get_lookup_table() -> Array[int]:
		return [
			0, 9, 57, 96, 135, 179, 236, 314, 419, 560,
			742, 973, 1_261, 1_612, 2_035, 2_535, 3_120, 3_798, 4_575, 5_460,
			6_458, 7_577, 8_825, 10_208, 11_735, 13_411, 15_244, 17_242, 19_411, 21_760,
...
class Slow extends GrowthRate:
	func _get_lookup_table() -> Array[int]:
		return [
			0, 10, 33, 80, 156, 270, 428, 640, 911, 1_250,
			1_663, 2_160, 2_746, 3_430, 4_218, 5_120, 6_141, 7_290, 8_573, 10_000,
...
class Erratic extends GrowthRate:
	func _get_lookup_table() -> Array[int]:
		return [
			0, 15, 52, 122, 237, 406, 637, 942, 1_326, 1_800,
			2_369, 3_041, 3_822, 4_719, 5_737, 6_881, 8_155, 9_564, 11_111, 12_800,
	...
	func apply_formula(level: int) -> int:
		if level < 50:
			...
			return (pow(level, 3) * floor((1911 - 10*level) / 3)) / 500
		else:
			return (pow(level, 3) * (160 - level)) / 100


class Fluctuating extends GrowthRate:
	func _get_lookup_table() -> Array[int]:
		return [
			0, 4, 13, 32, 65, 112, 178, 276, 393, 540,
			745, 967, 1_230, 1_591, 1_957, 2_457, 3_046, 3_732, 4_526, 5_440,
			6_482, 7_666, 9_003, 10_506, 12_187, 14_060, 16_140, 18_439, 20_974, 23_760,
	...
	func apply_formula(level: int) -> int:
		if level < 15:
			return (pow(level, 3) * floor((level + 1) / 3) + 24) / 50
		elif level < 36:
			return (pow(level, 3) * (level + 14)) / 50
		else:
			return ((pow(level, 3) * floor(level / 2)) + 32) / 50
